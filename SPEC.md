...
Reconcile all stores. Creates missing symlinks, replaces broken ones, reports conflicts. A staged render that was modified outside stitch (per the render journal) is an error, not an overwrite: the edit survives on disk, the entry is skipped, and apply exits non-zero with the recovery paths in the per-entry message.
...
Use `--source <repo-path>` to register a target path with an explicit repo-relative source (the fan-in flow: one file, many names/homes). Nothing is moved or copied; the target path must be absent or already a symlink into this repo (the adopt-a-link case). The source must be a safe repo-relative fragment, not under `.stitch/` or `.git/`, and not a symlink or through one (one hop only). Whole-directory stores cannot declare sources. The target's owning store is inferred from the longest matching target directory. If the target name is currently a `files` entry on the owning store, `add --source` migrates it in one atomic `state.toml` write: the `files` entry is dropped and the `sources` entry inserted (the files→sources conversion), with no unlink of the live target link — the next `apply` repoints it. The migration is reported in text output and as `migrated_from_files: true` in JSON.
...
After the editor exits successfully, `stitch apply` runs so everything converges — one command, edit, done. The full apply (not a single-entry re-render) is deliberate: one source edit can affect many consumers (the hub fan-in: an `include()`d hub edit must re-render every template that includes it). The edit is saved to disk before apply runs; an apply failure afterwards surfaces as an apply error, never as a lost edit — including a hand-edit refusal, where the recovery is to port the change into the source (done, if you just edited it) and then delete the staged render, since a non-byte-identical port still triggers the refusal. A non-zero editor exit skips the apply half entirely. `--print-path` is the agent path: print, exit, no editor, no apply (agents run `stitch apply` themselves when done editing).
...
Health check: missing store dirs, broken symlinks, conflicting targets, empty stores (suppressed for sources-only stores, whose empty store dir is the intended fan-in consumer shape), orphaned behavior (store in `stitch.toml` with no `state.toml` entry), staging permission drift, template staging content drift (staged ≠ fresh render; attributed as a `staging-hand-edited` error when the render no longer matches the render journal — apply will refuse to overwrite it), template render errors. It errors on a missing `.stitch/render/` gitignore entry only when configured templates are active on this platform or staged output exists; plain pre-template repos need no migration.
...
- **Staging and target links are reconciled and tool-owned.** `apply` removes staged renders and their stitch-owned target links when a source no longer resolves; `remove` deletes the store's staging tree alongside its links. Links to foreign destinations are never removed. The stale-link sweep walks the target directory but never descends into the repository itself, so a file-mode store with `target = "~"` (whose target contains the repo) does not classify or remove repo-internal organizational symlinks. Writes are hash-gated: unchanged content preserves mtime.
- **Hand-edited renders are refused, never overwritten (the render journal).** A target symlink points into `.stitch/render/`, so `$EDITOR <target>` edits the staged file directly — and the render tree is gitignored, making that edit the only copy. stitch records the sha256 of every render it writes in `.stitch/render/.journal.toml`; when a staged file differs from a fresh render *and* no longer matches its journal entry, `apply` (and `apply --plan`, which aborts at plan validation) refuses with the recovery paths and exits non-zero, `diff`'s dry run mirrors the refusal (both the re-render path and the whole-dir promotion preview), and `doctor` reports a `staging-hand-edited` error finding instead of plain drift. The stale-staging sweep refuses to *delete* a hand-edited render (e.g. after a template rename) — restore the source entry or delete the staged file deliberately. Crash semantics, stated precisely: a **missing** journal entry (pre-journal repo, or a crash that left none) is trusted once; a **stale** entry (crash between render write and journal write) is indistinguishable from a hand-edit and refuses — the safe direction, and deleting the staged file is harmless there because stitch rendered it. Journal entries are dropped when their staged file or store is removed. Honest limits: attribution is by content hash, so a hand edit that exactly matches the fresh render is converged, not a conflict; and recovery from a refusal requires porting the change into the source *and* deleting the staged file whenever the port is not byte-identical (the refusal message says so).
...
| 8 | `render` | Template render failed | address the render error above — full per-entry details in the plan output (JSON: `ops[].message`) |
