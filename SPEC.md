...
- **Hand-edited renders are refused, never overwritten (the render journal).** A target symlink points into `.stitch/render/`, so `$EDITOR <target>` edits the staged file directly — and the render tree is gitignored, making that edit the only copy. stitch records the sha256 of every render it writes in `.stitch/render/.journal.toml`; when a staged file differs from a fresh render *and* no longer matches its journal entry, `apply` (and `apply --plan`) refuses with the recovery paths (port the edit via `stitch edit`, or delete the staged file to discard it deliberately) and exits non-zero, `diff`'s dry run mirrors the refusal, and `doctor` reports a `staging-hand-edited` error finding instead of plain drift. A missing entry (repos staged before the journal, or a crash between write and journal) is trusted once — the journal never blocks convergence, it only refuses *unattributed* overwrites. Journal entries are dropped when their staged file or store is removed. Note the honest limits: attribution is by content hash, so a hand edit that exactly matches the fresh render is converged, not a conflict.
