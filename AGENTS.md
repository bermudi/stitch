...
- **Hand-edited staged renders are refused, never overwritten or swept.** A templated target symlinks into the gitignored `.stitch/render/` tree, so editing it through the target creates the only copy of that edit. stitch journals the sha256 of every render it writes (`.stitch/render/.journal.toml`, tool-owned, at the render root so sweeps never prune it); a staged file that differs from a fresh render *and* from its journal entry makes apply/`apply --plan`/`diff` refuse (exit non-zero, per-entry message with recovery), makes the stale-staging sweep refuse to delete it (template renames included), and doctor report `staging-hand-edited`. A missing journal entry is trusted once; a stale one (crash between render and journal write) is indistinguishable from a hand-edit and refuses — safe direction. Recovery from a refusal = port into the source + delete the staged render when the port isn't byte-identical. `stitch edit` is a full round-trip: editor on the repo source, then a full apply (hub fan-in means one edit affects many consumers). (2026-09-20, shipped.)
...
(d56aa75), `v0.8.0`, `v0.9.0`, `v0.10.0`, `v0.11.0`, `v0.11.1`, `v0.11.2`,
`v0.11.3`, `v0.11.4`, `v0.11.5`, `v0.11.6`, `v0.12.0`, `v0.13.0`, `v0.14.0`,
`v0.14.1`, `v0.14.2`.
