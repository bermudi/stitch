...
- **Hand-edited staged renders are refused, never overwritten.** A templated target symlinks into the gitignored `.stitch/render/` tree, so editing it through the target creates the only copy of that edit. stitch journals the sha256 of every render it writes (`.stitch/render/.journal.toml`, tool-owned, at the render root so sweeps never prune it); a staged file that differs from a fresh render *and* from its journal entry makes apply/`apply --plan`/`diff` refuse (exit non-zero, per-entry message with recovery: `stitch edit` to port, delete the staged file to discard) and doctor report `staging-hand-edited`. A missing journal entry is trusted once — the journal never blocks convergence, it only refuses unattributed overwrites. `stitch edit` is a full round-trip: editor on the repo source, then a full apply (hub fan-in means one edit affects many consumers). (2026-09-20, shipped.)
